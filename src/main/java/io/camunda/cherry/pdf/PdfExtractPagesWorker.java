...
public class PdfExtractPagesWorker extends PdfWorker {
  ...
  public PdfExtractPagesWorker() {
    super(WORKERTYPE_PDF_EXTRACTPAGES, Arrays.asList(
            ...
            RunnerParameter.getInstance(INPUT_EXTRACT_EXPRESSION, "Extract expression", String.class,
            ...
            RunnerParameter.getInstance(INPUT_DESTINATION_FILE_NAME, "File name,", String.class,
                RunnerParameter.Level.REQUIRED, "Destination file name"),
            RunnerParameter.getInstance(INPUT_DESTINATION_STORAGEDEFINITION, "Destination Storage definition", String.class,
            ...
            RunnerParameter.getInstance(OUTPUT_DESTINATION_FILE, "Destination variable name", Object.class,
  ...
  public void execute(final JobClient jobClient, final ActivatedJob activatedJob, ContextExecution contextExecution) {
    ...
    try {
      ...
      for (String oneExpression : expressionsList) {
        ...
        try {
          ...
          if (firstPage == -1 || lastPage == -1 || firstPage > lastPage)
            throw new ZeebeBpmnError(BPMERROR_INVALID_EXPRESSION,
                "Worker [" + getName() + "] Expression is <firstPage>-<lastPage> where firstPage<=lastPage :"
                    + firstPage + "," + lastPage);
        } catch (Exception e) {
          throw new ZeebeBpmnError(BPMERROR_INVALID_EXPRESSION,
              "Worker [" + getName() + "] Expression must be <firstPage>-<lastPage> : received[" + oneExpression + "] "
                  + e);
        }
      ...
      saveOutputPdfDocument(destinationDocument, destinationFileName, OUTPUT_DESTINATION_FILE,
          destinationStorageDefinition, contextExecution);
    ...
    logInfo("Cherry.PdfExtractPages: extract " + nbPagesExtracted + " pages from document["
